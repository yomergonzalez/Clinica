...
class Migration_Add_forenkey extends CI_Migration {
    ...
    public function down() {
        // Table config_roles
        $this->db->query('ALTER TABLE `medical`.`config_roles` DROP FOREIGN KEY `fk_clasificaciones_config_roles_1`;');
        $this->db->query('ALTER TABLE `medical`.`config_roles` DROP FOREIGN KEY `fk_controllers_config_roles_1`;');
        $this->db->query('ALTER TABLE `medical`.`config_roles` DROP FOREIGN KEY `fk_methods_config_roles_1`;');
        
        // Table Users
        $this->db->query('ALTER TABLE `medical`.`users` DROP FOREIGN KEY `fk_users_clasificaciones`;');
    }
